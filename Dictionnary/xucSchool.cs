...
{
    public partial class xucSchool : xucBaseBasic
    {
        public xucSchool()
        {
            InitializeComponent();
        ...
        protected override void Add()
        {
            if (!(MyRule.Get(MyLogin.RoleId, "bbiSchool") != "OK"))
            {
                if (MyRule.AllowAdd)
                {
                    xfmSchoolAdd _xfmSchoolAdd = new xfmSchoolAdd(Actions.Add);
                    _xfmSchoolAdd.Added += new xfmSchoolAdd.AddedEventHander(this.frm_Added);
                    _xfmSchoolAdd.ShowDialog();
                }
                else
                {
                    MyRule.Notify();
        ...
        public override void Change()
        {
            if (!(MyRule.Get(MyLogin.RoleId, "bbiSchool") != "OK"))
            {
                if (MyRule.AllowAccess)
                {
                    ...
                    object cellValue = base.GetCellValue(this.m_RowClickEventArgs.RowIndex, "SchoolCode");
                    if (cellValue != null)
                    {
                        base.SetWaitDialogCaption("Đang kiểm tra dữ liệu....");
                        if (!(dICSCHOOL.Get(cellValue.ToString()) != "OK"))
                        {
                            this.DoHide();
                            xfmSchoolAdd _xfmSchoolAdd = new xfmSchoolAdd(Actions.Update, dICSCHOOL);
                            _xfmSchoolAdd.Updated += new xfmSchoolAdd.UpdatedEventHander(this.frm_Updated);
                            _xfmSchoolAdd.Added += new xfmSchoolAdd.AddedEventHander(this.frm_Added);
                            _xfmSchoolAdd.ShowDialog();
                        }
                        else
        ...
        public override void Delete()
        {
            ...
            if (!(MyRule.Get(MyLogin.RoleId, "bbiSchool") != "OK"))
            {
                if (MyRule.AllowDelete)
                {
                    ...
                    {
                        flag = true;
                        cellValue = base.GetCellValue(selectedRows[i - 1], "SchoolCode");
                        if (cellValue != null)
                        {
                           // SYS_LOG.Insert("Danh Mục Trường Học", "Xoá", cellValue.ToString());
                            str = dICSCHOOL.Delete(cellValue.ToString());
                            if (str == "OK")
                            {
                    ...
                    if (!flag)
                    {
                        if (advBandedGridView.DataSource != null)
                        {
                            ...
                            cellValue = null;
                            cellValue = base.GetCellValue(rowClickEventArg.RowIndex, "SchoolCode");
                            if (cellValue == null)
                            {
                                return;
                            }
        ...
        protected override bool ExportPermision()
        {
            ...
            {
             //   SYS_LOG.Insert("Danh Mục Trường Học", "Xuất");
        ...
        private void frm_Added(object sender, DIC_SCHOOL e)
        {
            this.AddRow(e);
        }

        private void frm_Updated(object sender, DIC_SCHOOL Item)
        {
        ...
        protected override void gbList_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            RowClickEventArgs rowClickEventArg = new RowClickEventArgs((advBandedGridView == null ? -1 : advBandedGridView.FocusedRowHandle), (advBandedGridView.FocusedColumn == null ? -1 : advBandedGridView.FocusedColumn.ColumnHandle), (advBandedGridView.FocusedColumn == null ? "" : advBandedGridView.FocusedColumn.FieldName));
            this.m_RowClickEventArgs = rowClickEventArg;
            object cellValue = base.GetCellValue(rowClickEventArg.RowIndex, "SchoolCode");
        ...
        protected override void Print()
        {
            if (!(MyRule.Get(MyLogin.RoleId, "bbiSchool") != "OK"))
            {
                ...
                {
                  //  SYS_LOG.Insert("Danh Mục Trường Học", "In");
                    base.Print();
                }
                else
                {
        ...
        public event xucSchool.AddedEventHander Added;

        public event CHBK2014_N9.Common.Class.DeletedEventHander Deleted;

        public event xucSchool.UpdatedEventHander Updated;
