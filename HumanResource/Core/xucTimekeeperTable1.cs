...
{
    public partial class xucTimekeeperTable1 : xucBase
    {
        ...
        private void bbeTimeKeeperTableListName_EditValueChanged(object sender, EventArgs e)
        {
            ...
            this.gcList.DataSource = hRMTIMEKEEPERTABLE.GetList1(this.m_Level, this.m_Code, mTimeKeeperTableListID);
            ...
            string[] str = new string[] { "Bảng Chấm Công Tháng ", this.m_Month.ToString(), " - ", this.m_Year.ToString(), " (Theo Ký Hiệu - Dạng 2)" };
        ...
        {
            //xfmOptionPrintTimekeeperTable _xfmOptionPrintTimekeeperTable = new xfmOptionPrintTimekeeperTable(this.m_Level, this.m_Code, this.m_Month, this.m_Year, 1);
        ...
        public bool DeleteData()
        {
            ...
            flag = (!(hRMTIMEKEEPER.DeleteByEmployeeDate(this.m_TimeKeeperTableListID, this.m_EmployeeCode, this.m_Date) != "OK") ? true : false);
            return flag;
        }
        ...
        public void LoadGrid(int Level, string Code)
        {
            try
            {
                Guid mTimeKeeperTableListID = this.m_TimeKeeperTableListID;
                HRM_TIMEKEEPER_TABLE hRMTIMEKEEPERTABLE = new HRM_TIMEKEEPER_TABLE();
                this.gcList.DataSource = hRMTIMEKEEPERTABLE.GetList1(Level, Code, this.m_TimeKeeperTableListID);
                this.gbList.BestFitColumns();
            }
            catch
            {
        ...
        private void LoadInterface()
        {
            try
            {
                ...
                dataSet.ReadXml(string.Concat(Application.StartupPath, "\\LayoutControl\\xucTimekeeperTable1.xml"));
        ...
        public bool SaveData()
        {
            ...
            {
                if (hRMTIMEKEEPER.UpdateSymbol(this.m_TimeKeeperTableListID, row["EmployeeCode"].ToString(), row["ShiftCode"].ToString(), Convert.ToDateTime(row["Date"].ToString()), row["Symbol"].ToString(), Convert.ToDateTime(row["TimeIn"].ToString()), Convert.ToDateTime(row["TimeOut"].ToString()), Convert.ToDouble(row["Hour"].ToString()), Convert.ToDouble(row["DayHour"].ToString()), Convert.ToDouble(row["NightHour"].ToString()), Convert.ToDouble(row["PrivateHour"].ToString()), Convert.ToInt32(row["LateMinute"].ToString()), Convert.ToInt32(row["EarlyMinute"].ToString()), Convert.ToBoolean(row["IsOvertime"].ToString()), Convert.ToBoolean(row["IsWork"].ToString()), Convert.ToInt32(row["Sorted"].ToString()), row["Description"].ToString()) != "OK")
                {
                    flag = false;
                    return flag;
                }
        ...
        private void SaveStyle(bool ShowFooter, int HeightFooter)
        {
            ...
            try
            {
                dataSet.WriteXml(string.Concat(Application.StartupPath, "\\LayoutControl\\xucTimekeeperTable1.xml"));
            }
            catch
            {
            }
        ...
        public event xucTimekeeperTable1.ClosedHandler Closed;

        public event xucTimekeeperTable1.TimekeeperTableDeletedHandler TimekeeperTableDeleted;

        public event xucTimekeeperTable1.TimekeeperTableHourSelectedHandler TimekeeperTableHourSelected;

        public event xucTimekeeperTable1.TimekeeperTableInsertedHandler TimekeeperTableInserted;

        public event xucTimekeeperTable1.UnShiftDataHandler UnShiftData;
        ...
        private void bbeTimeKeeperTableListName_EditValueChanged_1(object sender, EventArgs e)
        {
            base.SetWaitDialogCaption("Đang khởi tạo dữ liệu...");
            HRM_TIMEKEEPER_TABLE hRMTIMEKEEPERTABLE = new HRM_TIMEKEEPER_TABLE();
            Guid mTimeKeeperTableListID = this.m_TimeKeeperTableListID;
        ...
        private void bbeTimeKeeperTableListName_EditValueChanged_2(object sender, EventArgs e)
        {
            try
            {
                DateTime dateTime = Convert.ToDateTime(this.bbeYear.EditValue.ToString());
        ...
        private void bbeYear_EditValueChanged_1(object sender, EventArgs e)
        {
            try
            {
                DateTime dateTime = Convert.ToDateTime(this.bbeYear.EditValue.ToString());
        ...
        private void bbiFinish_ItemClick_1(object sender, ItemClickEventArgs e)
        {
            if (ClsOption.System2.IsQuestion)
            {
                if (XtraMessageBox.Show(string.Concat("Bạn có chắc là muốn thực hiện tác vụ này không?", Environment.NewLine, " Tác vụ này được hiểu là bạn đã hoàn thành bảng chấm công của tháng này!"), "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
        ...
        private void bbiLock_ItemClick_1(object sender, ItemClickEventArgs e)
        {
            if (!((new HRM_TIMEKEEPER_TABLELIST()).Lock(this.m_TimeKeeperTableListID).ToString() == "OK"))
            {
                MessageBox.Show("Fail!");
        ...
        private void bbiOpen_ItemClick_1(object sender, ItemClickEventArgs e)
        {
            if (!((new HRM_TIMEKEEPER_TABLELIST()).Open(this.m_TimeKeeperTableListID).ToString() == "OK"))
            {
                MessageBox.Show("Fail!");
        ...
        private void bbiRefresh_ItemClick_1(object sender, ItemClickEventArgs e)
        {
            this.LoadGrid(this.m_Level, this.m_Code);
        }
        ...
        private void gbList_CellMerge_1(object sender, CellMergeEventArgs e)
        {
        ...
        private void gbList_CellValueChanged_1(object sender, CellValueChangedEventArgs e)
        {
            ...
            if ((e.Column == this.colRealDay || e.Column == this.colWorkTotal || e.Column == this.colCompensationDay || e.Column == this.colRegulationDay || e.Column == this.colFurloughDay || e.Column == this.colAbsentWOL || e.Column == this.colAbsentL || e.Column == this.colClockingError || e.Column == this.colClockingErrorBegin || e.Column == this.colClockingErrorEnd || e.Column == this.colTotalHour || e.Column == this.colLateMinute || e.Column == this.colEarlyMinute || e.Column == this.colNightDutyDay || e.Column == this.colExtraHour ? false : e.Column != this.colPrivateHour))
            {
                if (this.m_IsEditValueChanged)
                {
                    ...
                    if ((this.m_NewSymbol == "L" || this.m_NewSymbol == "CN" || this.m_NewSymbol == "T7" ? false : !(this.m_NewSymbol == "")))
                    {
                        this.m_IsWork = HRM_TIMEKEEPER_SHIFT.IsWork(this.m_TimeKeeperTableListID, this.m_EmployeeCode, this.m_ShiftCode, this.m_Date.Day);
                        string mNewSymbol = this.m_NewSymbol;
                        char[] chrArray = new char[] { ',' };
                        string[] strArrays = mNewSymbol.Split(chrArray);
                        for (int i = 0; i < (int)strArrays.Length; i++)
                        {
                            ...
                            {
                                if (!this.ShiftIsExist(strArrays1[0]))
                                {
                                    this.gbList.SetFocusedValue(this.m_Symbol);
                                    XtraMessageBox.Show("Ký hiệu chấm công không tồn tại!");
                                    return;
                                }
                                else
                                {
                                    this.m_ShiftCode = strArrays1[0];
                                    this.m_NewSymbol = "+";
                                    this.TotalCalculate(i, (int)strArrays1.Length);
                                    DataRowCollection rows = this.dt_Timekeeper.Rows;
                                    mTimeKeeperTableListID = new object[] { this.m_TimeKeeperTableListID, this.m_EmployeeCode, this.m_ShiftCode, this.m_Date, this.m_NewSymbol, this.m_TimeIn, this.m_TimeOut, this.m_Hour, this.m_DayHour, this.m_NightHour, this.m_PrivateHour, this.m_LateMinute, this.m_EarlyMinute, this.m_IsOvertime, this.m_IsWork, i, this.m_Description };
                                    rows.Add(mTimeKeeperTableListID);
                                }
                            }
                            else if ((int)strArrays1.Length == 3)
                            {
                                ...
                                {
                                    this.m_ShiftCode = strArrays1[0];
                                    this.m_NewSymbol = strArrays1[1];
                                    if (this.MultiSymbolIsExist())
                                    {
                                        string mNewSymbol1 = this.m_NewSymbol;
                                        chrArray = new char[] { ';' };
                                        string[] strArrays2 = mNewSymbol1.Split(chrArray);
                                        for (int j = 0; j < (int)strArrays2.Length; j++)
                                        {
                                            this.m_NewSymbol = strArrays2[j];
                                            this.TotalCalculate(j, (int)strArrays2.Length);
                                            DataRowCollection dataRowCollection = this.dt_Timekeeper.Rows;
                                            mTimeKeeperTableListID = new object[] { this.m_TimeKeeperTableListID, this.m_EmployeeCode, this.m_ShiftCode, this.m_Date, this.m_NewSymbol, this.m_TimeIn, this.m_TimeOut, this.m_Hour, this.m_DayHour, this.m_NightHour, this.m_PrivateHour, this.m_LateMinute, this.m_EarlyMinute, this.m_IsOvertime, this.m_IsWork, j, this.m_Description };
                                            dataRowCollection.Add(mTimeKeeperTableListID);
                                        }
                                    }
                                    else
                    ...
                    else
                    {
                        foreach (DataRow row in (new DIC_SHIFT()).GetList().Rows)
                        {
                            ...
                            this.TotalCalculate(0, 1);
                            DataRowCollection rows1 = this.dt_Timekeeper.Rows;
                            mTimeKeeperTableListID = new object[] { this.m_TimeKeeperTableListID, this.m_EmployeeCode, this.m_ShiftCode, this.m_Date, this.m_NewSymbol, this.m_TimeIn, this.m_TimeOut, this.m_Hour, this.m_DayHour, this.m_NightHour, this.m_PrivateHour, this.m_LateMinute, this.m_EarlyMinute, this.m_IsOvertime, false, 0, this.m_Description };
                            rows1.Add(mTimeKeeperTableListID);
                        }
                    }
                    this.DeleteData();
                    if (this.SaveData())
        ...
        private void gbList_CustomSummaryCalculate_1(object sender, CustomSummaryEventArgs e)
        {
        ...
        private void gbList_FocusedColumnChanged_1(object sender, FocusedColumnChangedEventArgs e)
        {
            try
            {
        ...
        private void gbList_MouseDown_1(object sender, System.Windows.Forms.MouseEventArgs e)
        {
