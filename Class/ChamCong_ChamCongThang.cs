...
namespace CHBK2014_N9.Class
{
    ...
    class ChamCong_ChamCongThang
    {
        ...
        public DataTable CT_TIMEKEEPER_TABLE_COMMENT_GetList()
        {
            string procname = "CT_TIMEKEEPER_TABLE_COMMENT_GetList";
            DbAccess db = new DbAccess();
            db.CreateNewSqlCommand();
            db.AddParameter("@TimeKeeperTableListID", TimeKeeperTableListID);
            DataTable dt = db.ExecuteDataTable(procname);           
        ...
        public bool CT_TIMEKEEPER_TABLE_COMMENT_Insert()
        {
            DbAccess db = new DbAccess();
            db.BeginTransaction();
            try
            {
                ...
                db.ExecuteNonQueryWithTransaction("CT_TIMEKEEPER_TABLE_COMMENT_Insert");
        ...
        public bool CT_TIMEKEEPER_TABLE_COMMENT_Update()
        {
            DbAccess db = new DbAccess();
            db.BeginTransaction();
            try
            {
                ...
                db.AddParameter("@Day", Day);
                db.AddParameter("@Description", Description);
                db.ExecuteNonQueryWithTransaction("CT_TIMEKEEPER_TABLE_COMMENT_Update");
                db.CommitTransaction();
                return true;
            }
            catch (Exception ex)
        ...
        public static DataTable TinhTongHopChamCong(DataTable dt,int year,int month,DayOfWeek days)
        {
            ...
            for(int i=0;i<dt.Rows.Count;i++)
            {
                ...
                for (int j = 1; j <= _Days; j++)
                    {
                        ...
                        if (dt.Rows[i]["D" + j].ToString().Equals("30PKL"))
                        {
                            _KL = _KL + 0.05F;
                        }
                        if (dt.Rows[i]["D" + j].ToString().Equals("60PKL"))
                        {
                            _KL = _KL + 0.1F;
                        }
                        if (dt.Rows[i]["D" + j].ToString().Equals("90PKL"))
                        {
                            _KL = _KL + 0.15F;
                        }
                        if (dt.Rows[i]["D" + j].ToString().Equals("120PKL"))
                        {
                            _KL = _KL + 0.2F;
                        }
                        if (dt.Rows[i]["D" + j].ToString().Equals("150PKL"))
                        {
                            _KL = _KL + 0.25F;
                        }

                        if (dt.Rows[i]["D" + j].ToString().Equals("180PKL"))
                        {
                            _KL = _KL + 0.3F;
                        }
                        if (dt.Rows[i]["D" + j].ToString().Equals("210PKL"))
                        {
                            _KL = _KL + 0.35F;
                        }
                        if (dt.Rows[i]["D" + j].ToString().Equals("240PKL"))
                        {
                            _KL = _KL + 0.4F;
                        }
                        if (dt.Rows[i]["D" + j].ToString().Equals("270PKL"))
                        {
                            _KL = _KL + 0.45F;
                        }
                        if (dt.Rows[i]["D" + j].ToString().Equals("300PKL"))
                        {
                            _KL = _KL + 0.5F;
                        }

                        if (dt.Rows[i]["D" + j].ToString().Equals("330PKL"))
                        {
                            _KL = _KL + 0.55F;
                        }
                        if (dt.Rows[i]["D" + j].ToString().Equals("360PKL"))
                        {
                            _KL = _KL + 0.6F;
                        }
                        if (dt.Rows[i]["D" + j].ToString().Equals("390PKL"))
                        {
                            _KL = _KL + 0.65F;
                        }
                        if (dt.Rows[i]["D" + j].ToString().Equals("420PKL"))
                        {
                            _KL = _KL + 0.7F;
                        }
                        if (dt.Rows[i]["D" + j].ToString().Equals("450PKL"))
                        {
                            _KL = _KL + 0.75F;
                        }
                        if (dt.Rows[i]["D" + j].ToString().Equals("480PKL"))
                        {
                            _KL = _KL + 0.8F;
                        }
                        if (dt.Rows[i]["D" + j].ToString().Equals("510PKL"))
                        {
                            _KL = _KL + 0.85F;
                        }
                        if (dt.Rows[i]["D" + j].ToString().Equals("540PKL"))
                        {
                            _KL = _KL + 0.9F;
                        }
                        if (dt.Rows[i]["D" + j].ToString().Equals("570PKL"))
                        {
                            _KL = _KL + 0.95F;
                        }
                        if (dt.Rows[i]["D" + j].ToString().Equals("600PKL"))
                        {
                            _KL = _KL + 1F;
                        }
                ...
                dt.Rows[i]["NC"] =SLNtrongThang- SNCN -_BH-_KL-_KP-_KLV;
                // tinh so ngay CN
                // cho Phep In
        ...
        public DataTable CT_TIMEKEEPER_TABLE_Create()
        {
            string procname = "CT_TIMEKEEPER_TABLE_Create";
            DbAccess db = new DbAccess();
            db.CreateNewSqlCommand();
            db.AddParameter("@TimeKeeperTableListID", TimeKeeperTableListID);
        ...
        public DataTable CT_TIMEKEEPER_TABLE_GetListByBranch()
        {
            string procname = "CT_TIMEKEEPER_TABLE_GetListByBranch";
            DbAccess db = new DbAccess();
            db.CreateNewSqlCommand();
            db.AddParameter("@TimeKeeperTableListID", TimeKeeperTableListID);
        ...
        public DataTable CT_TIMEKEEPER_TABLE_GetListByDepartment()
        {
            string procname = "CT_TIMEKEEPER_TABLE_GetListByDepartment";
            DbAccess db = new DbAccess();
            db.CreateNewSqlCommand();
            db.AddParameter("@TimeKeeperTableListID", TimeKeeperTableListID);
        ...
        public bool CT_TIMEKEEPER_TABLE_UpdateByShift()
        {
            DbAccess db = new DbAccess();
            db.BeginTransaction();
            try
            {
                ...
                db.AddParameter("@TimeKeeperTableListID", TimeKeeperTableListID);
                db.AddParameter("@EmployeeCode", EmployeeCode);
                db.ExecuteNonQueryWithTransaction("CT_TIMEKEEPER_TABLE_UpdateByShift");
                db.CommitTransaction();
                return true;
            }
            catch (Exception ex)
        ...
        public bool CT_TIMEKEEPER_TABLE_Delete()
        {
            DbAccess db = new DbAccess();
            db.BeginTransaction();
            try
            {
                db.CreateNewSqlCommand();
                db.AddParameter("@TimeKeeperTableListID", TimeKeeperTableListID);                
                db.ExecuteNonQueryWithTransaction("CT_TIMEKEEPER_TABLE_Delete");
                db.CommitTransaction();
                return true;
            }
            catch (Exception ex)
